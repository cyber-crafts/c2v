...
export default class ArrayValidator extends BaseTypeValidator {
  ...
  minItems (limit: number) {
    this.addValidator('minItems',async (value: any, obj: any, path: string, context: Context): Promise<void> => {
  ...
  maxItems (limit: number) {
    this.addValidator('maxItems',async (value: any, obj: any, path: string, context: Context): Promise<void> => {
